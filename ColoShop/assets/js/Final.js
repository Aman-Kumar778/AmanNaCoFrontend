// Event listener that runs after the DOM is fully loaded
document.addEventListener("DOMContentLoaded", function () {
  // Parsing URL parameters for the page and product ID
  const urlParams = new URLSearchParams(window.location.search);
  const page = urlParams.get("page");
  const id = urlParams.get("id");

  // Based on the page and ID in the URL, we call the respective function to display content
  if (page == 1 && id) {
    viewSingleProduct(id); // View the product details
  } else if (page == 2 && id) {
    viewSingleProductCart(id); // View the product in the cart
  } else if (page == 3 && id) {
    viewSingleProductCategory(id); // View the product category
  }
});

// Array to hold product data (currently unused)
let array = [];

// Base API URL for fetching products
const API_URL = "https://fakestoreapi.com";

// This variable keeps track of the currently selected product ID
let currentProductId = null;

// Function to update the cart item count displayed on the page
function Count() {
  let cartItems = JSON.parse(localStorage.getItem("cart")) || []; // Retrieve cart items from local storage
  let count = cartItems.length; // Count the number of items in the cart
  document.getElementById("cart-count").textContent = count; // Display the cart count
}

// Calling the Count function when the page loads
window.onload = Count;

// Function to show toast notifications with a message
function showToast(message, success = true) {
  const toaster = document.getElementById("toaster");

  // Check if toaster element exists before showing the toast
  if (toaster) {
    const toast = document.createElement("div");
    toast.className = "toast";

    // Set background color based on success or failure
    toast.style.backgroundColor = success ? "#F4D03F" : "#d9534f"; // Yellow for success, red for error
    toast.textContent = message; // Set the message text
    toaster.appendChild(toast); // Append the toast to the toaster container
    ...
    console.error("Toaster element not found!"); // Log an error if toaster is not found
  }
}
...
async function fetchProducts() {
  try {
    // Sending a request to the API and awaiting the response
    const response = await fetch(`${API_URL}/products`);
    const products = await response.json(); // Parse the response as JSON

    displayIndexProducts(products); // Call the function to display products on the page
    // Optionally show a success message (commented out here)
    // showToast("Products loaded successfully!", true);
  } catch (error) {
    // Handle any errors during the fetch process
    console.error("Error fetching products:", error);
    showToast("Failed to load products.", false); // Show error message in case of failure
  }
}
    ...
    if (bread_crumb) {
      bread_crumb.innerHTML = `
                <a href="./index.html" class="crumb">🏠 Home</a>
                 <span class="crumb" style="background: black;">🛍️ ${product.title}</span>
            `;
    }
...
async function viewSingleProductCart(productId) {
  try {
    ...
    if (bread_crumb) {
      bread_crumb.innerHTML = `
    <a href="./index.html" class="crumb">🏠 Home</a>
    <a href="./cart.html" class="crumb">🛒 Cart</a> 
    <span class="crumb" style="background: black;">🛍️ ${product.title}</span>
  `;
    }
...
async function fetchProductsByCategory(category) {
  try {
    ...
    if (bread_crumb) {
      bread_crumb.innerHTML = `
    <a href="./index.html" class="crumb">🏠 Home</a>
    <a href="./category.html" class="crumb">📂 Categories</a>
    <span class="crumb" style="background: black;">📁 ${category}</span>
  `;
    }
...
async function viewSingleProductCategory(productId) {
  try {
    ...
    <a href="./index.html" class="crumb">🏠 Home</a>
    <a href="./category.html" class="crumb">📂 Categories</a>
    <a onclick="showProductGridCategories()" class="crumb">📁 ${product.category}</a>
    <span class="crumb" style="background: black;">🛍️ ${product.title}</span>
  `;
    }
...
function showProductGridCategories() {
  ...
  if (bread_crumb) {
    ...
    <a href="./index.html" class="crumb">🏠 Home</a>
    <a href="./category.html" class="crumb">📂 Categories</a>
    <span class="crumb" style="background: black;">📁 ${currentCategory}</span>
  `;
  }
